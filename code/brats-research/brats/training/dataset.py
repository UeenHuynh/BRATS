...
def random_intensity_augmentation(image: NDArray[np.float32], rng: np.random.Generator) -> NDArray[np.float32]:
    """Apply mild MRI intensity perturbations when explicitly enabled."""
    if os.environ.get("BRATS_INTENSITY_AUGMENT", "0") != "1":
        return image
    foreground = image != 0
    ...
    result[foreground] = (values - mean) * float(rng.uniform(0.85, 1.15)) + mean + float(rng.uniform(-0.10, 0.10)) * std
    if rng.random() < 0.5:
        result[foreground] += rng.normal(0.0, 0.03 * std, size=int(foreground.sum()))
...
class VolumeDataset3D(Dataset[tuple[Tensor, Tensor]]):
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, Tensor]:
        ...
        if self.spec.augment:
            ...
            image = random_intensity_augmentation(image, self.rng)
